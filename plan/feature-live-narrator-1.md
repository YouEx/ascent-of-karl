...
| Opgave | Beskrivelse | Færdig | Dato |
|--------|-------------|--------|------|
| TASK-007 | Kør de genererede replikker gennem stemmedommeren fra narrationsplanens TASK-027-030. Genereret tekst er præcis den slags indhold, den plans RISK-001 handler om: den driver mod det generiske, og driften er usynlig, fordi hver enkelt replik ser rimelig ud. **Løsning:** `tools/voice/export_worker_profile.py` genererer `worker/src/generated/voice-profile.json` (version 1, indholdshash `c6a6dea3d960d490`) fra de samme frosne Python-artefakter som narrationsplanens dommer (fingeraftryk, leksikon, kalibrering) — DATA, ingen duplikeret prosa: tærskel (p5=0,8871), kontinuerlige fordelinger, ordforråds-/faste politiklister, generiske omkvæds-undtagelser, forbudte konstruktioner og moderne ordforråd bor alle i artefaktet, som `worker/src/voice/scorer.ts`+`gate.ts` læser ved import. Live-tekst får `source: "grammar"`-semantik (samme grammatik-regelsæt som narrationsplanens grammatikvarianter: maks 3 sætninger, maks 32 ord, forbudte fejlregister-vendinger, moderne ordforråd, meningsfuld omkvædsgenbrug, plus de samme kontinuerlige dimensioner og p5-tærsklen) — workerens EGNE par-specifikke krav (begge navne til stede, ingen pladsholdere, ≤320 tegn) er bevaret som et EKSTRA AND-gate ved siden af, ikke en erstatning. Ingen stille tilnærmelse: en Python-genereret paritetsfiksur (`tools/voice/export_voice_parity_fixture.py` → `tests/fixtures/voice-parity-fixture.json`, 2.101 sager: alle 866 håndskrevne varianter, alle 312 grammatikvarianter, alle 908 bagte par, 15 syntetiske hård-afvisnings-sager) beviser at TS-porten matcher Python på hårde afvisningsårsager/-kategorier PRÆCIST og samlet score inden for ±1e-4 (samme afrunding) — `tests/worker-voice-parity.test.ts` (6 tests). En stemme-afvist linje returnerer opstrøms-fejl (502), caches ALDRIG, og bruger det allerede-reserverede budget (modellen blev kaldt) — den afviste tekst optræder aldrig i logs/fejl. Cache-navnerummet (`cache-key.ts`s `promptNamespace()`) folder nu profilens version+hash ind, så en ændret stemmepolitik ugyldiggør gamle cache-poster automatisk. `tests/worker-voice-gate.test.ts` (19 tests) beviser lav-stemme-linjer afvist, gode linjer bestået, de hårde regler, at cachen aldrig gemmer en afvist linje, og at navnerummet ændrer sig med profilen. | ✅ | 2026-08-13 |
| TASK-008 | Høst de bedste live-replikker tilbage til bagte par: log hvilke par der oftest kaldes, og lad `tools/prepare_pairs.ts` foreslå dem som næste batch. Så bliver laget en **kilde til hånd­skrevet indhold**, ikke en erstatning for det. **Løsning:** `worker/src/stats.ts` tæller hver validerede, kanoniserede par+dom-forespørgsel (cache-hit ELLER -miss, inkl. budget-afviste forsøg — kun 400'ere tæller ikke) i Durable Object'ets egen storage, atomisk med den eksisterende koordinator-gate (`coordinator.ts`s `decide()`, fem exit-punkter) — bevidst SMAL post (`PairStatsRecord`): kun `aId`/`bId`/`verdict`/`count`/`cacheHits`/`upstreamCalls`/`firstSeen`/`lastSeen`, ALDRIG en IP (rå eller hashet) eller den genererede tekst. Et nyt, autentificeret admin-endpoint (`GET /admin/pairs` i den ydre worker) er beskyttet af en OBLIGATORISK `ADMIN_EXPORT_TOKEN`-secret med konstant-tids sammenligning (`worker/src/admin.ts`: SHA-256-hash begge sider, XOR-akkumuleret sammenligning) — omgår Origin (server-til-server, ikke browserkald) men kræver bearer-token, eksponerer aldrig IP-hashes/prompt-/modeltekst/hemmeligheder, er rate-limiteret via den SAMME mekanisme som narrator-strømmen (fast sentinel-nøgle, kan ikke kollidere med en rigtig IP-hash), og pagineret (`?limit=`/`?cursor=`, standard 200, hårdt loft 500). Den almindelige POST-strøm er upåvirket; admin-stien er GET-only og kan aldrig nå modellen eller røre budgettet (egen, tidlig dispatch i både `index.ts` og `coordinator-do.ts`, FØR POST-only-tjekket). `tools/live_pair_export.mjs` (+ `tools/live_pair_export_lib.mjs`) henter eksporten (miljøvariabler ALENE — aldrig CLI-argumenter — for token/URL), validerer skema+kanoniske id'er+domme, og skriver et lokalt, versioneret artefakt (`docs/design/live-pair-stats.json`, UDEN token) — fejler højlydt (ikke-nul exitkode) ved 401/uventet skema. `tools/prepare_pairs.ts` udvidet med en ikke-destruktiv høste-tilstand (`--live=<sti> --limit=<n> [--write] [--out=<sti>]`) der slår det simulerede signal sammen med rigtig trafik og rangerer **ukurerede** par+dom-nøgler til et forslag til næste bagerunde — uden `--write` KUN en JSON-forhåndsvisning på stdout, ingen diskskrivning; eksisterende simuleringsadfærd er bevaret byte-identisk (verificeret ved diff), ikke migreret væk. Forslag herfra går stadig igennem den SAMME menneske-gennemgang/`check_pairs.py`/`assemble_pairs.py` — intet auto-forfremmes. Retention: den eksisterende daglige oprydningsalarm rydder nu også stats-poster ældre end 90 dage (`STATS_MAX_AGE_MS`, baseret på `lastSeen`). Udrulning dokumenteret i `docs/deployment/live-narrator.md` afsnit 9 (ny, obligatorisk-kun-hvis-brugt secret, eksport-kommando, ingen følsomme data). Testdækket: `tests/worker-stats.test.ts` (29), `tests/worker-admin.test.ts` (13), `tests/worker-coordinator-stats.test.ts` (9), `tests/worker-admin-export.test.ts` (16), `tests/prepare-pairs-lib.test.ts` (24), `tests/live-pair-export.test.ts` (21). | ✅ | 2026-08-13 |

**CI-tillæg 13-08-2026:** Python 3.9 og 3.12 gengiver enkelte
tegnsætnings-scorer med forskel i sidste IEEE-754-bit. Fixturen blev derfor
byte-forskellig på en ren Linux-runner, selv om TS-paritetens tolerance er
`1e-4` og alle meningsbærende resultater var identiske. Eksportøren
kanoniserer nu de seks kontinuerte dimensioner til 12 decimaler — langt over
testens nødvendige præcision. En råformat-test pinner grænsen, og både Python
3.9 og 3.12 producerer nu samme fixture byte-for-byte.
