...
| TASK-029 | **Review-only stien er leveret som kode:** autentificeret `GET /admin/improvisations` eksporterer den aktuelle namespacede DO-cache i stabil leksikalsk par+akt-rækkefølge med cursor-after-key og efterspørgsels-/hit-/upstream-tællinger; `tools/harvest.mjs` kan hente alle sider eller læse en offline fixture, validere fjendtligt input og skrive atomisk til `content/drafts/harvested.json`. Output er ubetroede, manuelt rangerede review-kandidater og kan aldrig forfremmes automatisk. **Faktisk høst er fortsat blokeret** på en deployet Worker, et admin-token og rigtig trafik; der committes derfor intet fabrikeret harvest-output. | ◐ | 2026-08-13 |
