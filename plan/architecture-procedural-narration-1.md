...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-027 | Byg `tools/voice/metrics.py`: udled et stemmefingeraftryk fra de 71 håndskrevne replikker — ordlængdefordeling, sætninger pr. replik, andel af present tense, tegnsætningsrytme, ordforråd uden for korpus, forekomst af de faste figurer (Karl, vildsvinet, "Grub Man"). | | |
| TASK-028 | Byg `tools/voice/judge.py`: score enhver kandidatreplik mod fingeraftrykket, 0–1 pr. dimension. Afvis på: over 3 sætninger, over 32 ord, forbudte konstruktioner (fejlmeddelelses-tonefald: "cannot", "invalid", "try again"), moderne ordforråd, og genbrug af en punchline der allerede findes i korpusset. | | |
| TASK-029 | Kør dommeren over hele grammatikken (alle regler × alle varianter) og over de bagte replikker. Sæt tærsklen ud fra de håndskrevne replikkers egen score, ikke ud fra et ønsketal. | | |
| TASK-030 | Sæt dommeren i `npm run validate` som hård port. Ingen replik kommer i `content/` uden at bestå. Uenigheder mellem dommer og menneske logges i `docs/design/human-queue.json` efter dens eksisterende skema. | | |
| TASK-031 | Byg regressionstesten: simulér 200 runs, opsaml hver afgivet replik, og fejl hvis nogen replik optræder mere end 3 gange pr. run, eller hvis andelen af nødudgangsreplikker er over 0. **Bygget i `tests/narrator-regression.test.ts`. Nødudgangs-kravet holder (0 hits/200 runs). Gentagelsesloftet afslørede et rigtigt fund i stedet for at bestå: se opdateret RISK-002.** | ✅ | 2026-08-12 |
...
- **TEST-007**: 200 simulerede runs: ingen replik gentages mere end 3 gange i ét run; andelen af møder dækket af en *bagt* replik rapporteres og må ikke falde mellem to kørsler. ✅ 2026-08-12 — 200-run-delen bygget i `tests/narrator-regression.test.ts` (samme datasæt som TASK-031). Nødudgang: 0 hits/200 runs, holder. Gentagelsesloft: skrevet som rigtig, kørende test (`it.fails`), men afslørede at grammatikkens anti-gentagelse ikke overholder loftet i dag — se RISK-002. Bagt-dæknings-rapportering (sidste led) findes allerede som manuelt måltal i `tools/coverage_report.mjs` (TASK-026), ikke som automatiseret regressionsspærre; den del blev ikke bygget her.
- **TEST-008**: Stemmedommeren scorer de 71 håndskrevne replikker over tærsklen — hvis forfatterens egne replikker dumper, er dommeren forkert, ikke replikkerne.
...
- **RISK-001**: **Stemmeklippet.** Grammatikreplikker vil lyde fladere end de 71 håndskrevne, og kontrasten er synlig netop fordi de bedste replikker står lige ved siden af. Modtræk: TASK-015 bygger grammatikken af forfatterens egne dele, og fase 5 måler afstanden i stedet for at håbe.
- **RISK-002**: **Gentagelse dræber illusionen hurtigere end fladhed.** Med ~40 fiaskoer pr. run rammer selv 48 regler den samme replik flere gange. Modtræk: TASK-018 og TEST-007. **Bekræftet 2026-08-12 (TEST-007/TASK-031):** modtrækket er ikke nok. `recentGrammar` (K=6) er ét globalt vindue delt af alle 7 domme (~51 replikker), ikke ét pr. dom — når andre domme blander sig ind mellem to møder med samme dom, ekskluderes de færreste af den domms egne 6-8 replikker, og over 200 runs gentages en enkelt grammatik-id op til 5 gange i ét run (20 overtrædelser af 3-grænsen, spredt over `plausible`, `clash` og `near-miss`). Se `tests/narrator-regression.test.ts` for reproduktion; ingen rettelse er lavet her — det er en designbeslutning i `grammar.ts`/`narrator.ts` (fx vindue pr. dom), ikke en testopgave.
