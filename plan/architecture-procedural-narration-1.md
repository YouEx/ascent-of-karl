...
| TASK-031 | Byg regressionstesten: simulér 200 runs, opsaml hver afgivet replik, og fejl hvis nogen replik optræder mere end 3 gange pr. run, eller hvis andelen af nødudgangsreplikker er over 0. | | |
...
- **ALT-001**: **Infinite Craft-modellen** — lad en sprogmodel opfinde et *nyt element* for hvert par, så intet nogensinde fejler. Fravalgt: det opløser spillet. Karls historie har akter, obligatoriske problemer, skæbner, en krønike og 50 somre. Hvis alt lykkes, findes der ingen puslespil, ingen kuraterede opdagelser og ingen grund til at tænke. Martins ønske er at fiaskoen skal *fortælles*, ikke afskaffes.
...
- **TEST-007**: 200 simulerede runs: ingen replik gentages mere end 3 gange i ét run; andelen af møder dækket af en *bagt* replik rapporteres og må ikke falde mellem to kørsler.
...
- **TEST-009**: Hver grammatikregel har mindst 6 varianter, og ingen variant er identisk med en anden på tværs af domme.
- **TEST-010**: Bundtstørrelse: første indlæsning vokser ikke, og de lazy-loadede par pr. akt holder sig under 60 KB gzip (CON-003).
...
- **RISK-002**: **Gentagelse dræber illusionen hurtigere end fladhed.** Med ~40 fiaskoer pr. run rammer selv 48 regler den samme replik flere gange. Modtræk: TASK-018 og TEST-007.
