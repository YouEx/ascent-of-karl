...
| TASK-023 | Byg ruteren i `tools/judge/apply.mjs`: `token`→ skriv til `tuning.css`; `asset`→ tilføj til `docs/design/asset-queue.json` med spec og stop; `structure`→ skriv til arbejdskøen, anvend aldrig (CON-003). Landede i `apply.mjs`, ikke det oprindeligt planlagte `loop.mjs` (se FILE-006) — implementeret, bare aldrig afkrydset. **Revideret 2026-08-13** (2. anmeldelse, blokerer 1+2): `scopedOverall(scores, screenIds)` tilføjet — regionstal-vægtet gennemsnit begrænset til de anmodede skærme, så ufangede skærmes nulstubbe ikke længere kan fortynde en flerskærms-kørsels samlede score. `revert({tuningPath, backupPath})` gjort injicerbar, og CLI'ens `--revert` forstår nu `--run <dir>` (peger på den kørselsspecifikke `tuning.prev.css`) og et eksplicit `--backup <sti>` — før pegede den altid på én global, potentielt forældet sikkerhedskopi uanset hvilken kørsel der reelt fejlede. | ✅ | 2026-08-12 |
| TASK-024 | Implementér accept-porten: efter hver anvendelse køres optagelse + måling igen. Accepteres kun ved samlet forbedring **og** ingen region under −0,02 (CON-002). Ellers rulles `tuning.css` tilbage, og fundet markeres `rejected`. **Implementeret 2026-08-13** i `tools/judge/loop.mjs`s `acceptGate(before, after, {epsilon, maxDrop})`: accept kræver samlet fremgang > 0,002 og ingen region falder mere end 0,02. Ved afvisning genskrives `tuning.css` fra et in-memory råt strengsnapshot taget FØR anvendelse — aldrig `git reset`/`checkout` (det kunne ramme urelateret arbejde i en delt worktree). Bevist enhedstestet OG med en rigtig fixture-kørsel: en reelt målt titel-regression (chip −0,181, tools −0,105, tip-card −0,069) blev korrekt afvist, og `tuning.css` var byte-for-byte gendannet bagefter. **Revideret 2026-08-13** (2. anmeldelse, blokerer 1): `acceptGate` tager nu en valgfri `screenIds`-liste; når den er givet, beregnes gevinsten via `scopedOverall`, og regressionsscanningen begrænses til de samme skærme — en enkelt-skærms-kørsel bedømmes derfor ikke længere på en samlet score udvandet af nulstubbe for skærme, den aldrig optog. Uden `screenIds` er opførslen 100 % som før (bagudkompatibel). Ny test genskaber en titel-alene-gevinst omkring epsilon-grænsen mod en registry med flere skærme. | ✅ | 2026-08-13 |
| TASK-025 | Implementér journalen `.judge/<run>/ledger.json`: pr. iteration gemmes fund, anvendt ændring, før/efter-scorer, accepteret/fortrudt. Afviste fund fodres tilbage til dommeren som "dette er prøvet og gjorde det værre". Dette er hukommelsen fra punkt 5 i indledningen. **Implementeret 2026-08-13**: journalen ligger pr. kørsel, ikke i en global fil. Hver iteration gemmer fulde før/efter-scorer, fund, anvendte/forsøgte tokens, kø-resultater, herkomst (`consolidatedFrom`), verdikt, årsag og tidsstempler. Afviste nøgler samles i `ledger.rejected` og sendes med til dommerens næste prompt. Bevist med en rigtig accept-kørsel (gevinst +0,0035) og en rigtig afvist kørsel (samme værktøj, modsat token-værdi, fald −0,0125 globalt) — se verifikationsafsnittet. **Revideret 2026-08-13** (2. anmeldelse, blokerer 2+4): enhver undtagelse EFTER `tuning.css` er skrevet (i genoptagelse, måling, accept-port eller journalskrivning) fanges nu i `loop.mjs`, genskaber `tuning.css` byte-for-byte fra før-iterationens snapshot, journalfører et `crashed`-punkt (årsag, før-scorer, fund, forsøgte ændringer) hvis journalstien findes, og kaster den oprindelige fejl videre uændret — aldrig en tavs eller pyntet fejltilstand. `ledger.outcome` skelner nu desuden mellem `partial` (mindst én reel accept, men loftet nået) og `defeat` (ingen accepter); en stribe på tre afviste iterationer i træk er dog ALTID `defeat`, uanset tidligere accepter. 32 tests i `tests/judge-loop.test.ts` (op fra 22), inklusive tre dedikerede nedbrudsscenarier. **Revideret 2026-08-13** (2. anmeldelse, opfølgning): udfalds-matrixen var ufuldstændig — en kørsel med mindst én accept, der BAGEFTER blev blokeret af `NO_ACTIONABLE_TOKENS` (kun asset/struktur-fund tilbage), endte forkert som `defeat` på linje med "intet virkede nogensinde", selvom den havde bevaret reel fremgang. `ledger.outcome` behandler nu `NO_ACTIONABLE_TOKENS` som `MAX_ITERATIONS`: begge giver `partial`, når `acceptedCount > 0`. En tre-i-træk afvist streak (`NO_ACCEPT_STREAK`) forbliver bevidst `defeat` uanset tidligere accepter — se den nye kommenterede udfalds-matrix i `loop.mjs` for hele tabellen. 38 tests i `tests/judge-loop.test.ts` (op fra 32). | ✅ | 2026-08-13 |
| TASK-026 | Implementér stopbetingelser: alle regioner over tærsklen, eller tre iterationer i træk uden accept, eller 12 iterationer (CON-001). Sidste to rapporteres som nederlag med den bedste opnåede tilstand og de blokerende fund. **Implementeret 2026-08-13**: `decideStop()`/`resolveMaxIterations()` i `loop.mjs` — success når alle anmodede regioner er over tærsklen, nederlag ved tre på hinanden følgende afviste iterationer (nulstillet af en accept), ved det hårde loft på 12 (eller et eksplicit `--max`), eller når der ingen anvendelige token-fund er (kun asset/struktur-fund i kø, for at undgå at sløjfen spinder uden mål). Hvert nederlag rapporteres med den bedste opnåede tilstand (`bestTuning`/`bestScores`) og de blokerende fund. 22 tests i `tests/judge-loop.test.ts`, inklusive begge fixture-kørsler. **Revideret 2026-08-13** (2. anmeldelse, blokerer 3+4): `createCapture` er nu injicerbar (`startServerFn`/`launchBrowser`/`loadRegistryFn`) og rydder selv op, hvis browser- eller registry-opsætningen fejler efter serveren er startet — lukker browseren (hvis den nåede at åbne) og dræber altid serveren, før fejlen kastes videre. `main()` har `createCapture`-kaldet inde i sin beskyttede levetid med et hejst `dispose`. `npm run judge:once`s exitkode er nu 0 for en accepteret iteration, der rammer `--max 1` (`partial`), og først ikke-nul når INGEN iteration blev accepteret (`defeat`) — tidligere blev enhver `--max 1`-kørsel med præcis én iteration behandlet som nederlag, uanset reel fremgang. Test-tallet for filen (32) er opgjort under TASK-025's revision ovenfor. **Revideret 2026-08-13** (2. anmeldelse, opfølgning): `createCapture`s `dispose()` fangede ikke en fejlende `browser.close()` under NORMAL oprydning (kun i opsætnings-fejlstien fra forrige revision) — en sådan fejl ville forhindre `server.kill()` i at køre og, hvis den boblede op gennem `main()`s `finally`, overskrive kørslens EGEN fejl med en sekundær oprydningsfejl (JavaScripts finally-semantik). `dispose()` fanger nu selv `browser.close()`-fejl (logget, ikke kastet) før `server.kill()`, og en ny eksporteret `safeDispose(disposeFn)` bruges i `main()`s `finally` som ekstra sikkerhedslag. Test-tallet (38) er opgjort under TASK-025's revision ovenfor. | ✅ | 2026-08-13 |
...
| TASK-028 | Tilføj `npm run judge` (fuld sløjfe) og `npm run judge:report` (åbner sidste kørsels HTML-rapport med overlejringer og scorer side om side). **Implementeret 2026-08-13**: `npm run judge` kører den fulde sløjfe (`loop.mjs`), og `npm run judge:report` genererer en selvstændig `.judge/<run>/report.html` (`tools/judge/report.mjs`) — ingen CDN/netværksafhængighed, al modeltekst escapes før indsættelse. **Afviger bevidst fra planteksten**: rapporten åbner ikke længere automatisk — kun med eksplicit `--open` — den printer stien og returnerer, af hensyn til CI/headless-sikkerhed. Rapporten viser skærme, seneste reference/render/overlay/diff-billeder, scoretabel, tærskelstatus, hver iterations fund/ruter/før-efter-delta/verdikt/årsag, afvist hukommelse, køede blokeringer og stopårsag. 17 tests i `tests/judge-report.test.ts`, plus rigtige rapporter genereret og inspiceret fra begge fixture-kørsler. **Revideret 2026-08-13** (2. anmeldelse, blokerer 5): en `crashed`-iteration får nu sin egen farve og etiket ("forsøgt (rullet tilbage ved nedbrud)") i rapporten, adskilt fra både `rejected` og den generiske ukendte-status-farve; `ledger.outcome`s farve skelner nu visuelt `success`/`partial`/`crashed`/`defeat`. Åbne-kommandoen er udtrukket til en ren, eksporteret og enhedstestet `resolveOpenCommand(sti, platform)`: macOS bruger `open`, Windows bruger `cmd.exe /c start "" <sti>` (den tomme titel er påkrævet — ellers læser `start` en citeret sti som vinduestitel), øvrige platforme `xdg-open`. En fejlende `--open` sluges ikke længere stille i en tom callback — fejlen logges. 23 tests i `tests/judge-report.test.ts` (op fra 17). | ✅ | 2026-08-13 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-029 | Skriv accepterede regionsscorer til `tests/visual-baseline.json` med det commit, de blev målt på. **Lukket 13-08-2026:** baseline schema 1 identificerer commit `429849dd2ba4d483946c844f91facc96b68df49b`, maxDrop 0,02 og begge registrerede native skærme. Frisk genmåling i `.judge/visual-close-final` gav game 0,8528115 og title 0,7838425 (samlet 0,816512); regionsværdierne er pinnet til fire decimaler i baselinefilen. 429849d ændrede register/kø/test, ikke UI-renderen, og genmålingen reproducerede derfor den run, committen dokumenterer. | ✅ | 2026-08-13 |
| TASK-030 | Tilføj `tests/visual.test.ts` der kører optagelse + måling og fejler, hvis en region falder mere end 0,02 under baseline. Markeres som langsom test, kører ikke i `npm test`s hurtige sti. **Lukket 13-08-2026:** `npm run test:visual` bruger en isoleret `vitest.visual.config.ts`, kører den rigtige `capture.mjs` (build + `vite preview` + Chromium) og `metrics.py`, kræver samme skærm-/regionsidentitet som baselinen og fejler ved `drop > 0,02`. Rød-bevis rekonstrueret ved midlertidigt at sætte game/app-frame-baselinen til 0,95: den rigtige måling 0,8790 gav det forventede fald 0,0710 og testen fejlede; gendannet baseline bestod. `vite.config.ts` udelukker filen fra den hurtige sti. Capture-levetiden har egne tests og venter nu på preview-processens exit på både succes og fejl. | ✅ | 2026-08-13 |
