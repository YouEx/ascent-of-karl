...
Portrættet er manuelt godkendt og kan promoveres eksplicit. Den brede
kandidat er fail-closed: scriptet bygger målinger og kompakt evidens, men
skriver den aldrig til `docs/design/reference/`, fordi det bedste klassiske
resultat stadig har et synligt source-reuse-problem i den nederste join.
"""
...
def retention_metrics(
    ...
    sx, sy, width, height = metadata["sourceCrop"]
    dx, dy = metadata["sourcePlacement"]
    expected = source[sy : sy + height, sx : sx + width]
    actual = output[dy : dy + height, dx : dx + width]
    visible = metadata["hiddenMask"] == 0
    equal = np.all(actual == expected, axis=2)
    return {
        "retainedShare": float(np.mean(equal[visible])),
        "visiblePixels": int(np.sum(visible)),
        "changedVisiblePixels": int(np.sum(~equal & visible)),
    }
...
def repeat_metrics(
    ...
    return {
        "sourceReuseCorrelation": normalized_correlation(generated, donor),
...
def build_contact_sheet(
    ...
    draw.text((10, 12), "wide candidate - blocked", fill=(238, 224, 205))
    draw.text((980, 12), "portrait - approved", fill=(238, 224, 205))
    path.parent.mkdir(parents=True, exist_ok=True)
...
def build(
    ...
    if promote_portrait:
        promoted = resolve(config["portrait"]["output"])
        promoted.parent.mkdir(parents=True, exist_ok=True)
        temporary = promoted.with_name(f".{promoted.name}.tmp")
        shutil.copyfile(portrait_path, temporary)
    ...
    build_issue_overlay(wide, wide_metadata, issue_path)

    wide_retention = retention_metrics(wide, scene, wide_metadata)
    portrait_retention = retention_metrics(portrait, scene, portrait_metadata)
    ...
    repeated = repeat_metrics(wide, wide_metadata)
    gates = config["gates"]
    portrait_pass = (
        portrait_retention["retainedShare"] >= gates["sourceRetentionMin"]
        and portrait_karl["maxDelta"] <= gates["karlMaxDelta"]
        and all(
            seam["ratio"] <= gates["seamGradientRatioMax"]
            for seam in portrait_seams.values()
        )
        and config["portrait"]["promotion"]["approved"]
    )
    wide_pass = (
        wide_retention["retainedShare"] >= gates["sourceRetentionMin"]
        and wide_karl["maxDelta"] <= gates["karlMaxDelta"]
        and all(
            seam["ratio"] <= gates["seamGradientRatioMax"]
            for seam in wide_seams.values()
        )
        and repeated["sourceReuseCorrelation"]
        <= gates["repeatCorrelationMax"]
        and config["wide"]["promotion"]["approved"]
    )
    manifest = {
        ...
        "outputs": {
            ...
            "promotedPortrait": config["portrait"]["output"]
            if promote_portrait
            else None,
        },
        "metrics": {
        ...
        "promotion": {
            "wide": {
                "pass": wide_pass,
                **config["wide"]["promotion"],
            },
            "portrait": {
                "pass": portrait_pass,
                **config["portrait"]["promotion"],
            },
        },
        "evidence": {
            "contactSheet": str(contact_path.relative_to(ROOT))
            if contact_path.is_relative_to(ROOT)
            else contact_path.name,
            "wideBlockerOverlay": str(issue_path.relative_to(ROOT))
            if issue_path.is_relative_to(ROOT)
            else issue_path.name,
        },
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / ".judge/title-scene-masters/current",
    )
    parser.add_argument(
        "--evidence-dir",
        type=Path,
        default=ROOT / ".judge/title-scene-masters/evidence",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=ROOT / ".judge/title-scene-masters/manifest.json",
    )
    parser.add_argument("--promote-portrait", action="store_true")
    args = parser.parse_args()
    manifest = build(
        args.output_dir.resolve(),
        args.evidence_dir.resolve(),
        args.manifest.resolve(),
        args.promote_portrait,
    )
    print(json.dumps(manifest, indent=2))
    return 0 if manifest["promotion"]["portrait"]["pass"] else 1
