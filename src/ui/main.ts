...
function renderGrid(): void {
  ...
  for (const def of engine.availableElements()) {
    ...
    btn.addEventListener("click", () => selectElement(def));
    el.grid.appendChild(btn);
  }
}

function renderJournal(): void {
  el.journalList.innerHTML = engine
    .availableElements()
    .filter((d) => !d.base)
    .map(
      (d) => `
    ...
    .join("") || "<p>Ingen opdagelser endnu. Prøv at kombinere noget!</p>";
}

// Tap/klik-interaktion, ingen drag (PRD §2.1)
...
function showDiscoveryCard(outcome: Extract<CombineOutcome, { kind: "discovery" }>): void {
  ...
  el.card.innerHTML = `
    <div class="card-inner">
      ...
      <button id="card-close">Videre</button>
    </div>`;
